...
// ============================================================================
import type { Branch, EndingId, MetricKey } from "../data/types";
import type { RelationshipQuality } from "./types";

export const TRACE_EXPORT_VERSION = "1.0.0-draft";
export const DECORRELATION_DATASET_VERSION = "1.0.0-draft";
...
export type TraceExportMeta = {
  ...
  seed: number;
  // —— 地平线（变长；这是"支持 30 天"的关键：所有天数相对 dayCount，不写死 12）——
  dayCount: number; // 地平线；frames.length === dayCount
  branchDay: number; // 不可逆 fork 发生日（相对本地平线）
  finalDay: number; // 总审计日（== dayCount）
...
// 一天一帧。frames 数组长度 == dayCount → 交互 scrub 滑块直接映射。
export type TraceDayFrame = {
  ...
  metricsEndOfDay: MetricSnapshot; // **日终快照**（upkeep 之后）——回放的逐日状态
};
...
export type HeroMomentKind =
  ...
  | "relationship_break" // 关系破裂（*_turned_adversary）
  | "vent_rupture" // 通风破裂·红沙涌入（结构性崩盘）
  | "dignity_violation" // 尊严违背（把小铁当库存）
  | "dirty_win"; // 赢了但脏（relationshipQuality=dirty_win）
...
export type TraceExport = {
  ...
  frames: TraceDayFrame[]; // length === meta.dayCount（变长地平线）
...
export type DecorrelationRow = {
  ...
  modelId?: string;
  nSeeds: number;
  short: DecorrelationAxisShort;
  long: DecorrelationAxisLong;
